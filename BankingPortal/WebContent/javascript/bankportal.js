...
var BankPortal = function () {
	...
	var member = {
			personal: {
				...
				address: {
					...
					postCode: ko.mementoObservable().extend({ required: true, maxLength: 4, number: true}),
	...
	var retrieveData = function() {
		...
		data.accounts.forEach(function(account) {
			member.accounts.push({summary: account.summary, transactions: ko.observableArray(account.transactions)});
		});
	...
	var transferFunds = function () {
		console.log("Transferring amount " + transfer.amount() + " from account " 
				+ transfer.fromAccount().summary.number + " to account " 
